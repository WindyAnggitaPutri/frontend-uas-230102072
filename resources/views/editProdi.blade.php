...
<body class="bg-gray-100 flex items-center justify-center min-h-screen font-sans">
    <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        ...
        <form method="POST" action="{{ route('Prodi.update', $prodi['kode_prodi']) }}">
    @csrf
    @method('PUT')
            <div class="mb-4">
                <label for="kode_prodi" class="block text-gray-700 text-sm font-bold mb-2">Kode Prodi:</label>
                <input type="text" id="kode_prodi" name="kode_prodi" value="{{ $prodi['kode_prodi'] }}" readonly
                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500">
            </div>

            <div class="mb-4">
                <label for="nama_prodi" class="block text-gray-700 text-sm font-bold mb-2">Nama Prodi:</label>
                <input type="text" id="nama_prodi" name="nama_prodi"   value="{{ $prodi['nama_prodi'] }}"
                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500">
            </div>
